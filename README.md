...
透過 **AutoHotkey v2** 執行專案內的腳本，可在 MindMeister 分頁攔截右 Shift 並改送左 Shift，讓輸入法仍能切換中英文，同時降低網頁快捷鍵衝突。
...
| 功能 | 說明 |
|------|------|
| 防止 Add Media | 攔截右 Shift，不讓 MindMeister 收到，避免誤觸新增媒體 |
| 切換中英文 | 右 Shift 單獨按放 → 送出左 Shift → 微軟輸入法正常切換中英文 |
...
| 功能 | 操作 | 說明 |
|------|------|------|
| 防止 Add Media | 右 Shift（任意操作） | 攔截右 Shift，MindMeister 收不到 → 不觸發 Add Media |
| 切換中英文 | 右 Shift 單獨按放 | 系統看到 LShift 一按一放，微軟輸入法切換中英文 |
| **大寫英文** | 右 Shift 按住 + 字母 | 系統看到 LShift 持續按住 + 字母 → 打出大寫 |
| **全形/半形切換** | 右 Shift + 空格 | 系統看到 LShift + Space → 微軟輸入法切換全形與半形 |

> **v7 vs v8 技術差異**：v7 在右 Shift 按下瞬間就送出完整的 `{LShift}`（按＋放），然後等待放開；v8 改為按下送 `{LShift Down}`、放開送 `{LShift Up}`，讓整段持續期間系統都看到 Shift 被按住，從而支援 Shift 組合鍵。

### 適用範圍

- 腳本僅在 **MindMeister 分頁處於前景**時生效，不影響其他網頁或應用程式
- 僅攔截**右 Shift**，左 Shift 完全不受影響
...
1. 從本專案（GitHub）下載或複製檔案：  
   - 推薦使用 **v8**：`autokey_v8.ahk`  
   - 或使用基礎版：`autokey_v7.ahk`
2. 將腳本檔放在你方便管理的資料夾（例如 `文件\AutoHotkey\`），**請勿**放在會隨意刪除的暫存目錄。
3. 在檔案總管中 **按兩下** 腳本檔執行。
4. 若成功，工作列右側（系統匣）應會出現 **AutoHotkey 圖示**（綠色 **H**）。此時請在 **MindMeister 分頁** 測試：
   - 右 Shift 是否不再誤觸 Add Media
   - 輸入法是否仍可切換中英文
   - （v8）右 Shift + 字母是否能打出大寫
   - （v8）右 Shift + 空格是否能切換全形/半形
...
1. 在檔案總管中找到你的腳本檔（例如 `autokey_v8.ahk`）。
2. 對腳本檔按 **右鍵** → **顯示更多選項**（Windows 11）或直接選 **建立捷徑**。
3. 若建立的是「捷徑」在同一資料夾，請把該 **捷徑** 剪下並 **貼到** 上一小節開啟的「啟動」資料夾。
4. （選用）將捷徑重新命名為易懂的名稱，例如：`MindMeister RShift Fix`。
...
- 確認系統匣是否有 AutoHotkey 圖示；若沒有，請再按兩下腳本檔執行。
...
| 檔案 | 說明 |
|------|------|
| `autokey_v8.ahk` | **推薦** — 完整版腳本，支援大寫與全形/半形切換 |
| `autokey_v7.ahk` | 基礎版腳本，僅防止 Add Media 與中英切換 |
| `Requirement.md` | 需求文件與版本差異說明 |
